...
Promise.all([
    fetch("Academics_with_FAQs (1).json").then(r => r.json()),
    fetch("campusfacilities (1).json").then(r => r.json())
])
...
function resolveLocation(name) {
    ...
    Object.keys(LOCATION_MAPPINGS).forEach(key => {
        if (key === clean) {
            ...
            bestKeyScore = 1.0;
        } else if (clean.includes(key) && key.length > bestKeyScore) {
            resolvedDoorName = LOCATION_MAPPINGS[key];
            bestKeyScore = key.length * 0.1;
        }
    });
